...
const CareerPath = () => {
  return (
    <div className="container my-24">
       <h1 className="my-4 text-center font-medium  text-[30px] lg:text-[40px] text-slate-600">Career Path</h1>
    <div className=" grid grid-cols-2 sm:grid-cols-3 gap-3 xl:gap-4 ">
    {CareerPaths.map((item, i) => (
  <Card
    key={i}
    className={`group max-w-[350px] min-h-50  ${
      i === 4 ? "bg-slate-300 sm:bg-indigo-600 text-indigo-600 sm:text-white" : "bg-slate-300 text-indigo-600 hover:bg-indigo-500 hover:text-white"
  ...
  >
    <div className="relative">
      ...
      <div className="absolute top-5 z-40 left-1">
        <item.backIcon className={`text-[40px] ${i === 4 ? "text-white sm:text-indigo-700" : "text-white group-hover:text-indigo-600"}`} />
      </div>
    </div>
    <CardFooter>
